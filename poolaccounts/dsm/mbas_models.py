...
# SRAS related models
class MBASBaseModel(models.Model):
      ...
      class Meta:
            db_table = 'mbas_basemodel'
            unique_together=['Fin_year','Week_no','Entity']


class MBASPayments(models.Model):
      ...
      paystatus_fk=models.ForeignKey(MBASBaseModel,on_delete=models.SET_NULL,null=True)
      ...
      class Meta:
            db_table = 'mbas_payments'

class MBASReceivables(models.Model):
      Disbursed_amount=models.FloatField(blank=True,null=True)
      rcvstatus_fk=models.ForeignKey(MBASBaseModel,on_delete=models.SET_NULL,null=True)
