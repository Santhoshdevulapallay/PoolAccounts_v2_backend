...
# SRAS related models
class SRASBaseModel(models.Model):
      ...
      class Meta:
            db_table = 'sras_basemodel'
            unique_together=['Fin_year','Week_no','Entity']


class SRASPayments(models.Model):
      ...
      paystatus_fk=models.ForeignKey(SRASBaseModel,on_delete=models.SET_NULL,null=True)
      ...
      class Meta:
            db_table = 'sras_payments'

class SRASReceivables(models.Model):
      Disbursed_amount=models.FloatField(blank=True,null=True)
      rcvstatus_fk=models.ForeignKey(SRASBaseModel,on_delete=models.SET_NULL,null=True)
